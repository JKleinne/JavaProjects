 ...
 * Nom du fichier: PanneauImageBas.java
 ...
 * JKleinne 2021-11-27 15:32 p.m. Implemented translation (moving image)
...
/**
 * Création d'un panneau dans la partie inférieur de l'application pouvant contenir une image.
 * L'utilisateur peut modifier l'image au sein de ce panneau.
 * @author Équipe: 7
 * @since 2021-11-26
 */
public class PanneauImageBas extends JPanel implements Injectable, ITranslateElement, IRedimensionnerElement,
        IUndoElement, IRedoElement, IObserver {
    private static final long serialVersionUID = 1L;

    private Perspective clonePerspective;
    ...
    public void paint(Graphics g) {
        ...
        if(clonePerspective != null) {
            Point p = clonePerspective.getCoordinates();
            int height = clonePerspective.getImage().getHeight();
            int width = clonePerspective.getImage().getWidth();
            BufferedImage i = clonePerspective.getBufferedImage();
    ...
    private void translate(Point p) {
        if(clonePerspective != null) {
            ...
            translationMediator.notify(PanneauImageBas.this, payload);
        }
    }
    ...
    private void zoom(int i) {
        if(clonePerspective != null){
            ...
            zoomMediator.notify(PanneauImageBas.this, payload);
        }
    }
    ...
    public void inject(Object o) {
        if(o instanceof Perspective) {
            clonePerspective = (Perspective) o;

            clonePerspective.attach(this);
            clonePerspective.getImage().attach(this);
        }
    ...
    private boolean isMouseOnImage(MouseEvent e) {
        ...
        imageLocation = clonePerspective.getCoordinates();
        int height = clonePerspective.getImage().getHeight();
        int width = clonePerspective.getImage().getWidth();
        ...
        image.setBounds(clonePerspective.getCoordinates().x,clonePerspective.getCoordinates().y, width, height);
    ...
    public void accept(ITranslateVisitor v, Point coordinates) {
        v.visit(this.clonePerspective, coordinates);
    }
    ...
    public void accept(IRedimensionnerVisitor v, Pair<Integer, Integer> newDimensions) {
        v.visit(this.clonePerspective.getImage(), newDimensions);
    }
     ...
     */
    public Perspective getClonePerspective() {return clonePerspective;}
}
